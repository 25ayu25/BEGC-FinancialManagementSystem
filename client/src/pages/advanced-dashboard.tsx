...
function RevenueDataTable({ data, departments, monthName, selectedYear, selectedMonth, onClose }: RevenueDataTableProps) {
  ...
  const sortedData = [...tableData].sort((a, b) => {
    ...
    switch (sortField) {
      case "date": aVal = new Date(a.fullDate).getTime(); bVal = new Date(b.fullDate).getTime(); break;
      case "ssp": aVal = a.currency === "SSP" ? a.amount : 0; bVal = b.currency === "SSP" ? b.amount : 0; break;
      case "usd": aVal = a.currency === "USD" ? a.amount : 0; bVal = b.currency === "USD" ? b.amount : 0; break;
      case "total": aVal = a.amount; bVal = b.amount; break;
      case "department": aVal = a.departmentName; bVal = b.departmentName; break;
      default: aVal = a.fullDate; bVal = b.fullDate;
    }
  ...
  const getSortIcon = (field: SortField) => {
    ...
    return sortDirection === "asc" ? <ArrowUp className="h-3 w-3 text-slate-600" /> : <ArrowDown className="h-3 w-3 text-slate-600" />;
  };
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      <div className="flex-1 overflow-auto border border-slate-200 rounded-lg bg-white dark:bg-slate-800">
        <Table>
          <TableHeader className="sticky top-0 bg-white dark:bg-slate-800 z-10">
            <TableRow>
              <TableHead><Button variant="ghost" size="sm" onClick={() => handleSort("department")} className="font-semibold h-8 p-1">Department {getSortIcon("department")}</Button></TableHead>
              <TableHead><Button variant="ghost" size="sm" onClick={() => handleSort("date")} className="font-semibold h-8 p-1">Date {getSortIcon("date")}</Button></TableHead>
              <TableHead className="text-right"><Button variant="ghost" size="sm" onClick={() => handleSort("ssp")} className="font-semibold h-8 p-1 ml-auto">SSP {getSortIcon("ssp")}</Button></TableHead>
              <TableHead className="text-right"><Button variant="ghost" size="sm" onClick={() => handleSort("usd")} className="font-semibold h-8 p-1 ml-auto">USD {getSortIcon("usd")}</Button></TableHead>
              <TableHead className="text-right"><Button variant="ghost" size="sm" onClick={() => handleSort("total")} className="font-semibold h-8 p-1 ml-auto">Total {getSortIcon("total")}</Button></TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {sortedData.map((row, index) => (
              <TableRow key={row.id || index} className="hover:bg-slate-50 dark:hover:bg-slate-800">
                <TableCell className="text-sm font-medium text-teal-700 dark:text-teal-400">{row.departmentName}</TableCell>
                <TableCell className="text-sm font-medium">{row.fullDate}</TableCell>
                <TableCell className="text-sm font-mono tabular-nums text-right">{row.currency === "SSP" ? Math.round(row.amount).toLocaleString() : "—"}</TableCell>
                <TableCell className="text-sm font-mono tabular-nums text-right">{row.currency === "USD" ? row.amount.toLocaleString() : "—"}</TableCell>
                <TableCell className="text-sm font-mono tabular-nums text-right font-semibold">{Math.round(row.amount).toLocaleString()}</TableCell>
              </TableRow>
            ))}
            <TableRow className="bg-slate-100 dark:bg-slate-700 border-t-2 border-slate-300 dark:border-slate-600 font-semibold sticky bottom-0">
              ...
              <TableCell className="font-bold">Total</TableCell>
              <TableCell className="text-sm font-mono tabular-nums text-right font-bold">{Math.round(totals.ssp).toLocaleString()}</TableCell>
              <TableCell className="text-sm font-mono tabular-nums text-right font-bold">{totals.usd.toLocaleString()}</TableCell>
              <TableCell className="text-sm font-mono tabular-nums text-right font-bold">{Math.round(totals.ssp).toLocaleString()}</TableCell>
            </TableRow>
          </TableBody>
        </Table>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
        ...
        <div className="flex gap-2"><Button variant="outline" size="sm" onClick={onClose}>Close</Button></div>
      </div>
    </div>
  );
}
...
export default function AdvancedDashboard() {
  ...
  const handleTimeRangeChange = (range: "current-month" | "last-month" | "last-3-months" | "year" | "custom") => setTimeRange(range);
  ...
  const getPatientVolumeNavigation = () => {
    ...
    switch (timeRange) {
      ...
      case "last-month": { const d = new Date(currentDate.getFullYear(), currentDate.getMonth() - 1); return { year: d.getFullYear(), month: d.getMonth() + 1 }; }
      case "last-3-months": { const d = new Date(currentDate.getFullYear(), currentDate.getMonth() - 2); return { year: d.getFullYear(), month: d.getMonth() + 1 }; }
      case "year": return { year: currentDate.getFullYear(), month: 1 };
      case "custom": return customStartDate ? { year: customStartDate.getFullYear(), month: customStartDate.getMonth() + 1 } : { year: currentDate.getFullYear(), month: currentDate.getMonth() + 1 };
  ...
  if (timeRange === "custom" && customStartDate && customEndDate && Array.isArray(rawIncome)) {
    ...
    incomeSeries = Array.from({ length: daysInMonth }, (_, i) => ({
      ...
      amountSSP: 0,
      label: `${i + 1}`,
      fullDate: new Date(displayYear, displayMonth - 1, i + 1).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }),
    }));
  ...
  const formatXAxis = (tickItem: any, index: number) => {
    const day = Number(tickItem);
    const dayData = incomeSeries.find(d => d.day === day);
    const hasTransaction = !!dayData && (dayData.amountSSP > 0 || dayData.amountUSD > 0);
    const daysInCurrentMonth = incomeSeries.length;
    if (daysInCurrentMonth <= 28) return String(day);         // short months → show all
    if (hasTransaction) return String(day);                    // always show days with data
    return day % 5 === 0 ? String(day) : "";                   // otherwise every 5th day
  };
              ...
              <div>
                ...
                <div className="flex items-center mt-1">
                  {dashboardData?.changes?.incomeChangeSSP !== undefined && (
                    <span className={`text-xs font-medium ${dashboardData.changes.incomeChangeSSP > 0 ? "text-emerald-600" : dashboardData.changes.incomeChangeSSP < 0 ? "text-red-600" : "text-slate-500"}`}>
              ...
              <div>
                ...
                <div className="flex items-center mt-1">
                  {dashboardData?.changes?.expenseChangeSSP !== undefined && (
                    <span className={`text-xs font-medium ${dashboardData.changes.expenseChangeSSP > 0 ? "text-red-600" : dashboardData.changes.expenseChangeSSP < 0 ? "text-emerald-600" : "text-slate-500"}`}>
              ...
              <div>
                ...
                <div className="flex items-center mt-1">
                  {dashboardData?.changes?.netIncomeChangeSSP !== undefined && (
                    <span className={`text-xs font-medium ${dashboardData.changes.netIncomeChangeSSP > 0 ? "text-emerald-600" : dashboardData.changes.netIncomeChangeSSP < 0 ? "text-red-600" : "text-slate-500"}`}>
                ...
                <div>
                  ...
                  <div className="flex items-center mt-1">
                    {dashboardData?.changes?.incomeChangeUSD !== undefined ? (
                      <span className={`text-xs font-medium ${dashboardData.changes.incomeChangeUSD > 0 ? "text-emerald-600" : dashboardData.changes.incomeChangeUSD < 0 ? "text-red-600" : "text-slate-500"}`}>
            ...
            <CardContent className="p-4 sm:p-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center mt-1"><span className="text-xs font-medium text-teal-600">Current period</span></div>
                </div>
                  ...
                  <div className="ml-8 h-full w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        {/* Right Y axis (USD) only if USD exists */}
                        {hasAnyUSD && (
                          <YAxis
                            yAxisId="usd"
                            orientation="right"
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="flex flex-col text-center">
                      ...
                      <div className="space-y-1">
                        ...
                        {hasAnyUSD && <span className="block text-sm font-bold text-slate-900 font-mono tabular-nums">USD {monthTotalUSD.toLocaleString()}</span>}
                      </div>
                    </div>
                    <div className="flex flex-col text-center">
          ...
          <CardContent className="space-y-2">
            {Array.isArray(departments) ? departments
              ...
              .map((dept: any, index: number) => {
                ...
                return (
                  <div key={dept.id} className="w-full flex items-center justify-between p-3 rounded-lg border bg-slate-50 border-slate-100" data-testid={`row-department-${dept.id}`}>
                    ...
                    <div className="text-right flex-shrink-0 ml-4 min-w-[80px]">
                      <p className="font-semibold text-slate-900 text-sm font-mono tabular-nums">SSP {Math.round(dept.amount).toLocaleString()}</p>
            ...
            {Array.isArray(departments) && departments.length > 5 && (
              <Button variant="ghost" size="sm" className="w-full mt-2 text-slate-600 hover:text-slate-900 hover:bg-slate-50" onClick={() => setShowAllDepartments(!showAllDepartments)} data-testid="button-view-all-departments">
