...
function buildMonths(
  ...
  // custom
  if (cs && ce) {
    const s = new Date(cs.getFullYear(), cs.getMonth(), 1);
    const e = new Date(ce.getFullYear(), ce.getMonth(), 1);
    const list: MonthKey[] = [];
...
export default function InsuranceProvidersPage() {
  ...
  // ✅ Hardened monthly series: try/catch each month so one failure doesn't break the chart
  const { data: monthlySeries = [], isFetching: loadingMonthly } = useQuery({
    queryKey: [
      "/api/insurance/monthly-client",
      selectedYear,
      selectedMonth,
      timeRange,
      customStartDate?.toISOString(),
    ...
    queryFn: async () => {
      ...
      for (const { year, month } of months) {
        try {
          const resp = await fetch(`/api/dashboard?year=${year}&month=${month}`, {
            credentials: "include",
          });

          if (!resp.ok) {
            console.warn("monthly fetch not ok", { year, month, status: resp.status });
          ...
          const totalUSD = Object.values(br).reduce(
            (s, v) => s + (Number(v) || 0),
            0
          );
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Line
                    type="monotone"
                    dataKey={() =>
                      monthlySeries.reduce((s, r) => s + (r.usd || 0), 0) /
