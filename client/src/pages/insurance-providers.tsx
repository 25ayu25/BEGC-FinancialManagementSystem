...
import {
  Shield, DollarSign, TrendingUp, TrendingDown, ArrowLeft, Calendar as CalendarIcon,
} from "lucide-react";
...
export default function InsuranceProvidersPage() {
  ...
  const [customStartDate, setCustomStartDate] = useState<Date | undefined>(
    startDateParam ? new Date(`${startDateParam}T00:00:00Z`) : undefined
  );
  const [customEndDate, setCustomEndDate] = useState<Date | undefined>(
    endDateParam ? new Date(`${endDateParam}T00:00:00Z`) : undefined
  );
  ...
  // Current/selected period – use /api/insurance/breakdown
  const { data: breakdownData } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (timeRange === "custom" && customStartDate && customEndDate) {
        url = `/api/insurance/breakdown?start=${yyyymmdd(customStartDate)}&end=${yyyymmdd(customEndDate)}`;
      } else if (timeRange === "year") {
        url = `/api/insurance/breakdown?year=${selectedYear}`;
      } else if (timeRange === "last-3-months") {
        // Build a 3-month window anchored at selectedYear/selectedMonth
        const endMonthStart = new Date(Date.UTC(selectedYear, selectedMonth - 1, 1));
        const endInclusive  = new Date(Date.UTC(endMonthStart.getUTCFullYear(), endMonthStart.getUTCMonth() + 1, 0)); // last day of anchor month
        const startMonthStart = new Date(Date.UTC(endMonthStart.getUTCFullYear(), endMonthStart.getUTCMonth() - 2, 1));
        url = `/api/insurance/breakdown?start=${yyyymmdd(startMonthStart)}&end=${yyyymmdd(endInclusive)}`;
      } else {
        // "current-month", "last-month", "month-select" — fetch that single month
  ...
  // Comparison period (for the % change text and provider cards)
  const { data: comparisonBreakdown } = useQuery({
    queryKey: ["/api/insurance/breakdown", "comparison", selectedYear, selectedMonth, timeRange],
    queryFn: async () => {
      if (!(timeRange === "current-month" || timeRange === "last-month")) return null;

      let compYear = selectedYear;
      let compMonth = selectedMonth;

      if (timeRange === "current-month") {
        const lm = new Date(selectedYear, selectedMonth - 2); // previous month
        compYear = lm.getFullYear();
        compMonth = lm.getMonth() + 1;
      } else {
        // last-month -> compare to *current* month
  ...
  const prevInsuranceBreakdown: Record<string, number> = comparisonBreakdown?.breakdown || {};
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">with payments</p>
              </div>
            </div>
              ...
              <div className="h-[320px]">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    />
                    <Pie data={donutData} dataKey="value" nameKey="name" innerRadius={80} outerRadius={120} stroke="#ffffff" strokeWidth={2}>
        ...
        <Card className="border-0 shadow-md bg-white">
          ...
          <CardContent>
            <ul className="space-y-1">
              {monthlyInsurance.data.map((entry) => (
